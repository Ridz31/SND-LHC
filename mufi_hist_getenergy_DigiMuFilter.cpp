...
void mufi_hist_getenergy_DigiMuFilter(){
    ...
    std::vector<TH1D*> hist_vec_mufi_NC_en_00;
    std::vector<TH1D*> hist_vec_mufi_NC_en_01;
    std::vector<TH1D*> hist_vec_mufi_NC_en_02;
    std::vector<TH1D*> hist_vec_mufi_NC_en_03;
    std::vector<TH1D*> hist_vec_mufi_NC_en_04;
    // Fixed binning for Energy 
    for (int i = 0; i<(ctr_0)-1; i++){
        hist_vec_mufi_NC_en_00.push_back(new TH1D(("h_mufi_NC_en_00_"+std::to_string(i)).c_str(),";Energy[GeV];Number of events",10,0,2500)); 
    }
    for (int i = 0; i<(ctr_1)-1; i++){
        hist_vec_mufi_NC_en_01.push_back(new TH1D(("h_mufi_NC_en_01_"+std::to_string(i)).c_str(),";Energy[GeV];Number of events",10,0,2500));
    } 
    for (int i = 0; i<(ctr_2)-1; i++){
        hist_vec_mufi_NC_en_02.push_back(new TH1D(("h_mufi_NC_en_02_"+std::to_string(i)).c_str(),";Energy[GeV];Number of events",10,0,2500));
    } 
    for (int i = 0; i<(ctr_3)-1; i++){
        hist_vec_mufi_NC_en_03.push_back(new TH1D(("h_mufi_NC_en_03_"+std::to_string(i)).c_str(),";Energy[GeV];Number of events",10,0,2500));
    }
    for (int i = 0; i<(ctr_4)-1; i++){
        hist_vec_mufi_NC_en_04.push_back(new TH1D(("h_mufi_NC_en_04_"+std::to_string(i)).c_str(),";Energy[GeV];Number of events",10,0,2500));
    }
            ...
            if (TMath::Abs(((ShipMCTrack*)MCTracks->At(1))->GetPdgCode())==12||TMath::Abs(((ShipMCTrack*)MCTracks->At(1))->GetPdgCode())==14){
                ...
                if(ev_sta==0){ 
                    for( int i=0; i< numbin_0; i++){
                            if (bin_0.at(i)<=get_energy && bin_0.at(i+1)>get_energy) { 
                                cout << bin_0.at(i) << " " <<bin_0.at(i+1)<<" "<<i;
                                i_hist=i;}
                    }
                    cout<<i_hist<<endl;
                    cout<<hist_vec_mufi_NC_en_00.size()<<endl;
                    hist_vec_mufi_NC_en_00.at(i_hist)->Fill(enu_hadron);
                    cout<<"Hist station NC 0"<<i_hist<<endl;}
                if(ev_sta==1){                        
                    for( int i=0; i< numbin_1; i++){ 
                            if (bin_1.at(i)<=get_energy&& bin_1.at(i+1)>get_energy) { 
                                cout << bin_1.at(i) << " " <<bin_1.at(i+1)<<" "<<i;
                                i_hist=i;} 
                    }
                    cout<<i_hist<<endl;
                    cout<<hist_vec_mufi_NC_en_01.size()<<endl;
                    hist_vec_mufi_NC_en_01.at(i_hist)->Fill(enu_hadron);
                    cout<<"Hist station NC 1"<<i_hist<<endl;}
                if(ev_sta==2){
                    for( int i=0; i< numbin_2; i++){ 
                            if (bin_2.at(i)<=get_energy && bin_2.at(i+1)>get_energy) { 
                                cout << bin_2.at(i) << " " <<bin_2.at(i+1)<<" "<<i <<endl;
                                i_hist=i;} 
                    }
                    cout<<i_hist<<endl;
                    cout<<hist_vec_mufi_NC_en_02.size()<<endl;
                    hist_vec_mufi_NC_en_02.at(i_hist)->Fill(enu_hadron);
                    cout<<"Hist station NC 2"<<i_hist<<endl;}
                if(ev_sta==3){
                    for( int i=0; i< numbin_3; i++){ 
                            if (bin_3.at(i)<=get_energy && bin_3.at(i+1)>get_energy) { 
                                cout << bin_3.at(i) << " " <<bin_3.at(i+1)<<i;
                                i_hist=i;} 
                    }
                    cout<<i_hist<<endl;
                    cout<<hist_vec_mufi_NC_en_03.size()<<endl;
                    hist_vec_mufi_NC_en_03.at(i_hist)->Fill(enu_hadron);
                    cout<<"Hist station NC 3"<<i_hist<<endl;}
                if(ev_sta==4){
                    for( int i=0; i< numbin_4; i++){ 
                        if (bin_4.at(i)<=get_energy && bin_4.at(i+1)>get_energy) { 
                            cout << bin_4.at(i) << " " <<bin_4.at(i+1)<<i;
                            i_hist=i;} 
                    }
                    cout<<i_hist<<endl;
                    cout<<hist_vec_mufi_NC_en_04.size()<<endl;
                    hist_vec_mufi_NC_en_04.at(i_hist)->Fill(enu_hadron);
    ...
    TFile* fout3 = new TFile("/eos/user/r/ridz01/Hist_MuFi_NC_energy_getenergy_DigiMufi_irregular_231122_200files.root","RECREATE");
    for (int i_hist=0; i_hist<numbin_0; i_hist++){
        hist_vec_mufi_NC_en_00.at(i_hist)->Write();
    }
    for (int i_hist=0; i_hist<numbin_1; i_hist++){
        hist_vec_mufi_NC_en_01.at(i_hist)->Write();
    }
    for (int i_hist=0; i_hist<numbin_2; i_hist++){
        hist_vec_mufi_NC_en_02.at(i_hist)->Write();
    }
    for (int i_hist=0; i_hist<numbin_3; i_hist++){
        hist_vec_mufi_NC_en_03.at(i_hist)->Write();
    }
    for (int i_hist=0; i_hist<numbin_4; i_hist++){
         hist_vec_mufi_NC_en_04.at(i_hist)->Write();
    }
